...
function init() {
    ...
    document.getElementById("game-button").addEventListener("click", handleGameStateChange);
    document.getElementById("player-button").addEventListener("click", toggleUserPlayer);
    injectListeners();
    nonActive = true;
    userPlayer = "Y";
    gameId = 0;
        ...
        if (x >= i * 100 + 10 && x < i * 100 + 110) {
            for (let j = 0; j < ROWS; j++) {
                if (y >= j * 100 + 10 && y <= j * 100 + 110) {
                    if (inCircle(i * 100 + 60, j * 100 + 60, 40, x, y))
                        performPlayerMove(userPlayer, i);
                    return;
                }
            }
        }
...
function checkWinner() {
    ...
    for (let i = -3; i <= 3; i++) {
        if (
            i + lastPlayedRow < 0 ||
            i + lastPlayedRow >= 6 ||
            lastPlayedColumn + i < 0 ||
            lastPlayedColumn + i >= 7
        )
            continue;
